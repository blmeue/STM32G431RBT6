...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
      ...
      LCD_DisplayStringLine(Line9, (uint8_t *)text);//将text里面的数据打印到第九行
      
      if(key[0].single_flag==1){//第一个按键被按下
      sprintf(text,"Key0down       ");
      LCD_DisplayStringLine(Line8, (uint8_t *)text);//将text里面的数据打印到第九行
      ...
      if(key[3].single_flag==1){//第一个按键被按下
