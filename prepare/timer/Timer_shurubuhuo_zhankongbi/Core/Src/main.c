...
void key_proc(void)
{
   ...
   __HAL_TIM_SetCompare(&htim17,TIM_CHANNEL_1,pa6_duty);//设置占空比
   ...
   __HAL_TIM_SetCompare(&htim16,TIM_CHANNEL_1,pa7_duty);//设置占空比
...
void lcd_proc(void)
{
   ...
   //第五行显示定时器2的占空比,R40
   sprintf((char *)text,"tim2_duty:%0.3f %%",duty1);
  ...
  //第六行显示定时器3的占空比,R39
   sprintf((char *)text,"tim3_duty:%0.3f %%",duty2);
